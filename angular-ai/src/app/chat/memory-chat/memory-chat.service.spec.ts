...
describe('MemoryChatService', () => {
  ...
  describe('continueChatStream', () => {
    it('should POST to the stream endpoint and emit text deltas', () => {
      ...
      const req = httpMock.expectOne(`${service.API_MEMORY}/${chatId}/stream`);
      expect(req.request.method).toBe('POST');
      expect(req.request.body).toEqual({ message: 'Follow-up message' });
      req.flush('data: {"content":"Hel","type":"ASSISTANT"}\n\ndata: {"content":"lo","type":"ASSISTANT"}\n\n');
